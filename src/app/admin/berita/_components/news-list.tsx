...
export function NewsList() {
  ...
  const { data: news, isLoading } = useCollection<News>(newsQuery); console.log('NewsList render');
          ...
          {news?.length === 0 ? (
            ...
            (news || []).map((item) => (
              <TableRow key={item.id}>
                <TableCell>
                  <img src={item.imageUrl?.replace('/image/upload/','/image/upload/w_300,q_auto,f_auto/') || '/placeholder.jpg'} className="h-10 w-16 object-cover rounded border" loading="lazy" alt={item.title} />
                </TableCell>
                <TableCell>
                  <p className="font-bold text-sm line-clamp-1">{item.title}</p>
                  <p className="text-[10px] text-muted-foreground">{item.author}</p>
                </TableCell>
                <TableCell className="text-[10px] uppercase tracking-[0.15em] text-slate-500 font-bold">{item.mediaType === 'video' || item.videoUrl ? 'Video' : 'Foto'}</TableCell>
